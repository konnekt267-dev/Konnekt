...
function renderWizard(){
  ...
  if(!wizard.role){
    ...
    document.getElementById('wizardBody').innerHTML = `
      <p class="wizard-intro">Are you backing teams, or looking for backing?</p>
      <div class="role-choice">
        <button type="button" class="role-card" onclick="chooseWizardRole('sponsor')">
          <span class="role-emoji">🤝</span>
          <span class="role-name">Sponsor</span>
          <span class="role-desc">I want to back teams and projects</span>
        </button>
...
function renderWizardStepBody(key){
  ...
  if(key === 'location'){
    return `
      <div class="field full">
        <label for="wLocation">Location <span class="field-optional">(helps match you with nearby teams &amp; sponsors)</span></label>
  ...
  if(key === 'tags'){
    const hint = wizard.role === 'sponsor'
      ? "What are you interested in backing?"
      : "What are you interested in from sponsors?";
    return `
      <div class="field full">
        <label for="wTags">Tags <span class="field-optional">(comma-separated, up to 8)</span></label>
        <input type="text" id="wTags" value="${escapeHtml(wizard.tags)}" placeholder="${hint}">
        <p class="field-hint">You can change these any time in your profile settings.</p>
      </div>`;
  }
  if(key === 'display'){
    return `
...
function openListingDetail(id){
  ...
  document.getElementById('listingModalBody').innerHTML = `
    ...
    <div class="card-actions" style="margin-top:14px;">
      ...
      ${canProposeDeal ? `<button class="btn btn-cyan" onclick="openProposeDeal('${l.id}')">Propose a deal</button>` : ''}
...
function roleBadgeHtml(role){
  ...
  if(role === 'sponsee') return `<span class="badge team">TEAM / SPONSEE</span>`;
  return '';
}
...
function openProposeDeal(listingId){
  ...
  if(l.type === 'sponsor'){
    if(myRole && myRole !== 'sponsee'){ showToast("Only a team account can propose a deal on a sponsor listing."); return; }
    sponsorId = l.user_id; teamId = session.user.id;
  } else {
    if(myRole && myRole !== 'sponsor'){ showToast("Only a sponsor account can propose a deal on a team listing."); return; }
...
async function confirmDelivery(id){
  ...
  if(otherAlreadyConfirmed) payload.status = 'completed';
  payload.updated_at = new Date().toISOString();
  const { error } = await sb.from('deals').update(payload).eq('id', id);
  if(error){ showToast("Couldn't confirm — " + error.message); return; }
  showToast(payload.status === 'completed' ? "Deal completed! You can both leave a review now." : "Confirmed — waiting on the other side.");
  await loadDeals();
  renderDealsList();
}
