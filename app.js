...
function openEditListing(id){
  ...
  document.getElementById('fTags').value = (l.tags || []).join(', ');
  ...
  if(isOwn){
    body.innerHTML = `
      ...
      <form onsubmit="saveProfile(event)">
        ...
        <div class="field full">
          <label for="pTags">Tags <span class="field-optional">(comma-separated, up to 8)</span></label>
          <input type="text" id="pTags" value="${escapeHtml((profile.tags || []).join(', '))}" placeholder="e.g. Robotics, Youth programs">
        </div>
        <div class="field full">
